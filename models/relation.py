...
class TaskAwareRelation(nn.Module):
    def __init__(self, inp_dim, hidden_dim, num_layers, edge_n_layer, num_class=2,
                res_alpha=0., top_k=-1, node_concat=True, batch_norm=False, dropout=0.0,
                 adj_type='sim', activation='softmax',pre_dropout=0.0,mod=1,ar=1,gpu=0,adc=0,k_shot=10,lm=0):
        ...
        if self.adc==0:
            ...
            self.fc1 = nn.Sequential(nn.Linear(gnn_inp_dim, gnn_inp_dim), nn.LeakyReLU())
            if self.pre_dropout>0:
                self.predrop2 = nn.Dropout(p=self.pre_dropout)
            self.fc2 = nn.Linear(gnn_inp_dim, gnn_inp_dim)
        elif self.adc==2:
            if self.ar==1:
